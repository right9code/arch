...
# Snapper config creation will be done manually after first boot
# snapper -c root create-config / # Removed due to D-Bus issues in chroot
...
echo ""
echo "FINAL STEPS:"
echo " 1. Exit the chroot environment if you are still in it (type 'exit')echo " 2. Unmount all partitions: \'umount -R /mnt\' (run this multiple times if needed)."
...
echo " 6. AFTER FIRST BOOT: Run \'sudo snapper -c root create-config /\' to initialize Snapper."xit 0
